...
export default function SearchTrackRow({ track, allTracks }: SearchTrackRowProps) {
  const { activeTrack, isPlaying, setQueue, setIsPlaying, language } = usePlayer()
  ...
  return (
    <div
      ...
      className={`w-full h-14 rounded-lg px-4 flex items-center justify-between cursor-pointer group border transition-all duration-300 relative ${
    ...
    >
      ...
      <div className="flex items-center gap-x-3 truncate flex-1 mr-4">
        ...
        <div className="truncate">
          <h4 className={`text-xs font-black uppercase tracking-wide truncate transition-colors duration-300 ${
            isCurrentTrackPlaying ? 'text-white' : 'text-zinc-400 group-hover:text-white'
          }`}>
            {track.title}
          </h4>
